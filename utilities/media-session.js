...
import {
    FALLBACKIMAGE /* , PLAYER_SKIP_TIME */
} from "~/composables/globals"
import axios from 'axios'
import {
    RemoteStreamer
} from "@kimlarocca/capacitor-remote-streamer"
...
const fetchMimeType = async ( imageUrl ) => {
    try {
        const response = await axios( imageUrl, {
            method: 'HEAD'
        } ) // Use 'HEAD' to avoid downloading the image
...
const generateMediaSessionArtworkArray = async ( image ) => {
    ...
    const format = await fetchMimeType( imageSolver( image, {
        w: 116,
        h: 116,
        q: 80,
        format: 'jpeg'
...
export const initMediaSession = async ( episode /* , skipTime = PLAYER_SKIP_TIME */ ) => {
    ...
    const artworkImageArray = currentEpisode?.directoryImage?.uri & !isNetworkConnected.value ? [ {
        src: currentEpisode.directoryImage.uri
    } ] : await generateMediaSessionArtworkArray( currentEpisode.image )
